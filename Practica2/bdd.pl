...
% -----------------------------------------
menu_criterios :- write('Seleccione el criterio mas relevante'), nl,
...
% -------- PRESUPUESTO --------
presupuesto :- write('Monto del presupuesto: '),
    read(PRESUPUESTO), separar_presupuesto(PRESUPUESTO).

separar_presupuesto(PRESUPUESTO) :- (
    PRESUPUESTO < 1000 -> bajo_presupuesto(PRESUPUESTO);
    PRESUPUESTO >= 3000 -> alto_presupuesto(PRESUPUESTO);
    medio_presupuesto(PRESUPUESTO)
).

bajo_presupuesto(PRESUPUESTO) :- write('** Plan de viaje economico **'), nl,
    write('\tPresupuesto maximo a gastar en comida: '),
    read(COMIDA_MAX),
    write('\tPresupuesto maximo a gastar en la habitacion: '),
    read(HABITACION_MAX),
    write('\tOpinion minima de los clientes (0-10): '),
    read(OPINION_MIN),
    write('\tCantidad de personas a viajar: '),
    read(CANTIDAD_PERSONAS),
    write('\tTipo de habitacion (simple, doble): '),
    read(TIPO_HABITACION),
    write('\tCantidad de dias a hospedarse: '),
    read(CANTIDAD_DIAS),
    write('\tTiempos de comida (0-3): '),
    read(TIEMPOS_COMIDA),
    write('\tViaja en vehiculo (s/n): '),
    read(VEHICULO),
    inf_bajo_presupuesto(PRESUPUESTO, COMIDA_MAX, HABITACION_MAX, OPINION_MIN, CANTIDAD_PERSONAS, TIPO_HABITACION, CANTIDAD_DIAS, TIEMPOS_COMIDA, VEHICULO).

medio_presupuesto(PRESUPUESTO) :- write('** Plan de viaje medio **'), nl,
    write('\tClima del hotel (calor, frio, templado, tropical): '),
    read(CLIMA),
    write('\tDistancia maxima de viaje (km): '),
    read(DISTANCIA_MAX),
    write('\tCantidad de estrellas minima (1-5): '),
    read(ESTRELLAS),
    write('\tCantidad de personas a viajar: '),
    read(CANTIDAD_PERSONAS),
    write('\tTipo de habitacion (simple, doble): '),
    ...
    write('\tViaja en vehiculo (s/n): '),
    read(VEHICULO),
    inf_medio_presupuesto(PRESUPUESTO, CLIMA, DISTANCIA_MAX, ESTRELLAS, CANTIDAD_PERSONAS, TIPO_HABITACION, CANTIDAD_DIAS, TIEMPOS_COMIDA, VEHICULO).

alto_presupuesto(PRESUPUESTO) :- write('** Plan de viaje Premium **'), nl,
    write('\tClima del hotel (calor, frio, templado, tropical): '),
    read(CLIMA),
    write('\tEl hotel debe tener clientes extranjeros (s/n): '),
    read(CLIENTES_EXTRANJEROS),
    write('\tRequiere un chef (s/n): '),
    read(CHEF),
    write('\tCantidad de personas a viajar: '),
    read(CANTIDAD_PERSONAS),
    write('\tTipo de habitacion (simple, doble): '),
    ...
    (
        CLIENTES_EXTRANJEROS == 's' , CHEF == 's' -> inf_alto_presupuesto(PRESUPUESTO, CLIMA, _, 'Chef', CANTIDAD_PERSONAS, TIPO_HABITACION, CANTIDAD_DIAS, TIEMPOS_COMIDA, VEHICULO);
        CLIENTES_EXTRANJEROS == 's', CHEF == 'n' -> inf_alto_presupuesto(PRESUPUESTO, CLIMA, _, _, CANTIDAD_PERSONAS, TIPO_HABITACION, CANTIDAD_DIAS, TIEMPOS_COMIDA, VEHICULO);
        CLIENTES_EXTRANJEROS == 'n', CHEF == 's' -> inf_alto_presupuesto(PRESUPUESTO, CLIMA, 'Guatemala', 'Chef', CANTIDAD_PERSONAS, TIPO_HABITACION, CANTIDAD_DIAS, TIEMPOS_COMIDA, VEHICULO);
        CLIENTES_EXTRANJEROS == 'n', CHEF == 'n' -> inf_alto_presupuesto(PRESUPUESTO, CLIMA, 'Guatemala', _, CANTIDAD_PERSONAS, TIPO_HABITACION, CANTIDAD_DIAS, TIEMPOS_COMIDA, VEHICULO)
    ).


% -------- IDIOMA --------
idioma :- write('Idiomas disponibles (espanol, ingles, katchikel, ketchi): '),
    read(IDIOMA),
    write('\tDistancia maxima de viaje (km): '),
    read(DISTANCIA_MAX),
    write('\tTiempo maximo de viaje (h): '),
    read(TIEMPO_MAX),
    write('\tClima del hotel (calor, frio, templado, tropical): '),
    read(CLIMA),
    write('\tCantidad de personas a viajar: '),
...
% -------- ESTRELLAS --------
estrellas :- write('Cantidad de estrellas minima (1-5): '),
...
pocas_estrellas(ESTRELLAS) :- write('\tPresupuesto maximo a gastar en comida: '),
    read(COMIDA_MAX),
    write('\tDistancia maxima de viaje (km): '),
    read(DISTANCIA_MAX),
    write('\tPresupuesto maximo a gastar en la habitacion: '),
...
muchas_estrellas(ESTRELLAS) :- write('\tRequiere un chef (s/n): '),
    read(CHEF),
    write('\tClima del hotel (calor, frio, templado, tropical): '),
    read(CLIMA),
    write('\tIdiomas disponibles (espanol, ingles, katchikel, ketchi): '),
    read(IDIOMA),
    write('\tCantidad de personas a viajar: '),
    read(CANTIDAD_PERSONAS),
    write('\tTipo de habitacion (simple, doble): '),
...
% -------- CLIMA --------
clima :- write('Clima del hotel (calor, frio, templado, tropical): '),
    read(CLIMA),
    write('\tOpinion minima de los clientes (0-10): '),
    read(OPINION_MIN),
    write('\tTiempo maximo de viaje (h): '),
...
inf_alto_presupuesto(PRESUPUESTO, CLIMA, CLIENTES_EXTRANJEROS, CHEF, CANTIDAD_PERSONAS, TIPO_HABITACION, CANTIDAD_DIAS, TIEMPOS_COMIDA, VEHICULO) :- (
    % BUSQUEDA DE HECHOS
    hotel(ID_HOTEL, BD_NOMBRE_H, BD_DIREC_H, _, BD_HAB_SIMPLE_H, BD_HAB_DOBLE_H, BD_PRECIO_COMIDA_H, ID_DEPARTAMENTO, BD_DIS_H),
    departamento(ID_DEPARTAMENTO, BD_NOMBRE_D, _, _, BD_CLIMA, BD_PASAJE_D),
    registro(_, ID_CLIENTE, ID_HOTEL, _, _, _),
