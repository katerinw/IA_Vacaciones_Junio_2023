...
% ------------- Menu inicio -------------
% -----------------------------------------
inicio :- write('***** Bienvenido ***'), nl, nl,
	write('Selecciona una opcion'), nl,
	format('\t[1] Planificar viaje
    \t[2] Reportes
	-> '),
	read(OP_INICIO), opcion_inicio(OP_INICIO).
opcion_inicio(OP_INICIO) :- (
    OP_INICIO == 1 -> menu_criterios;
    OP_INICIO == 2 -> menu_reportes
).
...
menu_criterios :- write('Selecciona el criterio mas importante'), nl,
	format('\t[1] Presupuesto
    \t[2] Idioma
    \t[3] Cantidad de estrellas del hotel
    \t[4] Clima
	-> '),
	read(OP_CRITERIO), opcion(OP_CRITERIO).
	
opcion(OP_CRITERIO) :- (
	OP_CRITERIO == 1 -> presupuesto;
	OP_CRITERIO == 2 -> idioma;
	OP_CRITERIO == 3 -> estrellas;
	OP_CRITERIO == 4 -> clima
).
...
% ------------- Planificacion -------------
% -----------------------------------------
presupuesto :- write('Cual es tu presupuesto?'),
	read(PRESUPUESTO), tipos_presupuesto(PRESUPUESTO).
	
tipos_presupuesto(PRESUPUESTO) :- (
	PRESUPUESTO < 1000 -> presupuesto_bajo(PRESUPUESTO);
	PRESUPUESTO >= 3000 -> presupuesto_alto(PRESUPUESTO);
	presupuesto_medio(PRESUPUESTO)
).

presupuesto_bajo(PRESUPUESTO) :- write('** Plan de viaje economico **'), nl,
	write('\tCual es tu presupuesto maximo a gastar en comida?: '),
    read(COMIDA_MAX),
    write('\tCual es tu presupuesto maximo a gastar en la habitacion?: '),
    read(HABITACION_MAX),
    write('\tCual es la puntuacion minima de los clientes? (0-10): '),
    read(OPINION_MIN),
    write('\tCuantas personas viajaran?: '),
    read(CANTIDAD_PERSONAS),
    write('\tQue tipo de habitacion prefieres? (simple, doble): '),
    read(TIPO_HABITACION),
    write('\tCuantos dias se hospedaran?: '),
    read(CANTIDAD_DIAS),
    write('\tCuantos tiempos de comida incluiras diarios? (0-3): '),
    read(TIEMPOS_COMIDA),
    write('\tViajas en vehiculo? (s/n): '),
    read(VEHICULO),
    inf_presupuesto_bajo(PRESUPUESTO, COMIDA_MAX, HABITACION_MAX, OPINION_MIN, CANTIDAD_PERSONAS, TIPO_HABITACION, CANTIDAD_DIAS, TIEMPOS_COMIDA, VEHICULO).

medio_presupuesto_medio(PRESUPUESTO) :- write('** Plan de viaje simple **'), nl,
    write('\tQue clima prefieres en el hotel? (calor, frio, templado, tropical): '),
    read(CLIMA),
    write('\tCual es la distancia maxima del viaje? (km): '),
    read(DISTANCIA_MAX),
    write('\tCual es la cantidad de estrellas minima? (1-5): '),
    read(ESTRELLAS),
    write('\tCuantas personas viajaran?: '),
    read(CANTIDAD_PERSONAS),
    write('\tQue tipo de habitacion prefieres? (simple, doble): '),
    ...
    write('\tViajas en vehiculo? (s/n): '),
    read(VEHICULO),
    inf_medio_presupuesto_medio(PRESUPUESTO, CLIMA, DISTANCIA_MAX, ESTRELLAS, CANTIDAD_PERSONAS, TIPO_HABITACION, CANTIDAD_DIAS, TIEMPOS_COMIDA, VEHICULO).

presupuesto_alto(PRESUPUESTO) :- write('** Plan de viaje premium **'), nl,
    write('\tQue clima prefieres en el hotel? (calor, frio, templado, tropical): '),
    read(CLIMA),
    write('\tEl hotel debe tener clientes extranjeros? (s/n): '),
    read(CLIENTES_EXTRANJEROS),
    write('\tSe requiere un chef? (s/n): '),
    read(CHEF),
    write('\tCuantas personas viajaran?: '),
    read(CANTIDAD_PERSONAS),
    write('\tQue tipo de habitacion prefieres? (simple, doble): '),
    ...
    write('\tViajas en vehiculo? (s/n): '),
    read(VEHICULO),
    % Condiciones para clientes extranjeros o chefs
    (
        CLIENTES_EXTRANJEROS == 's' , CHEF == 's' -> inf_presupuesto_alto(PRESUPUESTO, CLIMA, _, 'Chef', CANTIDAD_PERSONAS, TIPO_HABITACION, CANTIDAD_DIAS, TIEMPOS_COMIDA, VEHICULO);
        CLIENTES_EXTRANJEROS == 's', CHEF == 'n' -> inf_presupuesto_alto(PRESUPUESTO, CLIMA, _, _, CANTIDAD_PERSONAS, TIPO_HABITACION, CANTIDAD_DIAS, TIEMPOS_COMIDA, VEHICULO);
        CLIENTES_EXTRANJEROS == 'n', CHEF == 's' -> inf_presupuesto_alto(PRESUPUESTO, CLIMA, 'Guatemala', 'Chef', CANTIDAD_PERSONAS, TIPO_HABITACION, CANTIDAD_DIAS, TIEMPOS_COMIDA, VEHICULO);
        CLIENTES_EXTRANJEROS == 'n', CHEF == 'n' -> inf_presupuesto_alto(PRESUPUESTO, CLIMA, 'Guatemala', _, CANTIDAD_PERSONAS, TIPO_HABITACION, CANTIDAD_DIAS, TIEMPOS_COMIDA, VEHICULO)
    ).

% -------- IDIOMA --------
idioma :- write('Elige un idioma disponible (espanol, ingles, katchikel, ketchi): '),
    read(IDIOMA),
    write('\tCual es la distancia maxima del viaje? (km): '),
    read(DISTANCIA_MAX),
    write('\tCual es el tiempo maximo de viaje? (h): '),
    read(TIEMPO_MAX),
    write('\tQue clima prefieres en el hotel? (calor, frio, templado, tropical): '),
    read(CLIMA),
    write('\tCuantas personas viajaran?: '),
    ...
    write('\tViajas en vehiculo? (s/n): '),
    read(VEHICULO),
    inf_idioma(IDIOMA, DISTANCIA_MAX, TIEMPO_MAX, CLIMA, CANTIDAD_PERSONAS, TIPO_HABITACION, CANTIDAD_DIAS, TIEMPOS_COMIDA, VEHICULO).

% -------- ESTRELLAS --------
estrellas :- write('\tCual es la cantidad de estrellas minima? (1-5): '),
    read(ESTRELLAS), separar_estrellas(ESTRELLAS).

separar_estrellas(ESTRELLAS) :- (
    ESTRELLAS =< 3 -> pocas_estrellas(ESTRELLAS);
    ESTRELLAS >= 4 -> muchas_estrellas((ESTRELLAS))
).

pocas_estrellas(ESTRELLAS) :- write('\tCual es tu presupuesto maximo a gastar en comida?: '), 
    read(COMIDA_MAX),
    write('\tCual es la distancia maxima del viaje? (km): '),
    read(DISTANCIA_MAX),
    write('\tCual es tu presupuesto maximo a gastar en la habitacion: '),
    ...
    write('\tViajas en vehiculo? (s/n): '),
    read(VEHICULO),
    inf_pocas_estrellas(ESTRELLAS, COMIDA_MAX, DISTANCIA_MAX, HABITACION_MAX, CANTIDAD_PERSONAS, TIPO_HABITACION, CANTIDAD_DIAS, TIEMPOS_COMIDA, VEHICULO).

muchas_estrellas(ESTRELLAS) :- write('\tRequeriras un chef? (s/n): '),
    read(CHEF),
    write('\tQue clima prefieres en el hotel? (calor, frio, templado, tropical): '),
    read(CLIMA),
    write('Elige un idioma disponible (espanol, ingles, katchikel, ketchi): '),
    read(IDIOMA),
    write('\tCuantas personas viajaran?: '),
    read(CANTIDAD_PERSONAS),
    write('\tQue tipo de habitacion prefieres? (simple, doble): '),
    ...
    write('\tViajas en vehiculo? (s/n): '),
    read(VEHICULO),
    % Condiciones para filtrado por hoteles con trabajadores chefs
    (
        CHEF == 's' -> inf_muchas_estrellas(ESTRELLAS, 'Chef', CLIMA, IDIOMA, CANTIDAD_PERSONAS, TIPO_HABITACION, CANTIDAD_DIAS, TIEMPOS_COMIDA, VEHICULO);
        CHEF == 'n' -> inf_muchas_estrellas(ESTRELLAS, _, CLIMA, IDIOMA, CANTIDAD_PERSONAS, TIPO_HABITACION, CANTIDAD_DIAS, TIEMPOS_COMIDA, VEHICULO)
    ).

% -------- CLIMA --------
clima :- write('\tQue clima prefieres en el hotel? (calor, frio, templado, tropical): '),
    read(CLIMA),
    write('\tQue opinion minima de los clientes quieres? (0-10): '),
    read(OPINION_MIN),
    write('\tCual es el tiempo maximo de viaje? (h): '),
    read(TIEMPO_MAX),
    write('\tCuantas personas viajaran?: '),
    ...
    write('\tViajas en vehiculo? (s/n): '),
    read(VEHICULO),
    inf_clima(CLIMA, OPINION_MIN, TIEMPO_MAX, CANTIDAD_PERSONAS, TIPO_HABITACION, CANTIDAD_DIAS, TIEMPOS_COMIDA, VEHICULO).
...
inf_presupuesto_bajo(PRESUPUESTO, COMIDA_MAX, HABITACION_MAX, OPINION_MIN, CANTIDAD_PERSONAS, TIPO_HABITACION, CANTIDAD_DIAS, TIEMPOS_COMIDA, VEHICULO) :- (
    % BUSQUEDA DE HECHOS
    hotel(ID_HOTEL, BD_NOMBRE_H, BD_DIREC_H, _, BD_HAB_SIMPLE_H, BD_HAB_DOBLE_H, BD_PRECIO_COMIDA_H, ID_DEPARTAMENTO, BD_DIS_H),
    departamento(ID_DEPARTAMENTO, BD_NOMBRE_D, _, _, _, BD_PASAJE_D),
    registro(_, _, ID_HOTEL, _, _, BD_OPINION_R),
    % CALCULOS
    (
          VEHICULO == 's' -> COSTO_TRANSPORTE is (BD_DIS_H * 12.50 * 2);
          VEHICULO == 'n' -> COSTO_TRANSPORTE is (BD_PASAJE_D * 2 * CANTIDAD_PERSONAS)
    ),
    COSTO_COMIDA is (BD_PRECIO_COMIDA_H * TIEMPOS_COMIDA * CANTIDAD_PERSONAS * CANTIDAD_DIAS),
    (
        TIPO_HABITACION == 'simple' -> COSTO_HABITACION is (BD_HAB_SIMPLE_H * CANTIDAD_PERSONAS * CANTIDAD_DIAS);
        TIPO_HABITACION == 'doble' -> COSTO_HABITACION is (BD_HAB_DOBLE_H * CANTIDAD_PERSONAS * CANTIDAD_DIAS)
    ),
    COSTO_TOTAL is (COSTO_TRANSPORTE + COSTO_COMIDA + COSTO_HABITACION),
    % RESTRICCIONES
    COSTO_TOTAL =< PRESUPUESTO,
    COSTO_COMIDA =< COMIDA_MAX,
    COSTO_HABITACION =< HABITACION_MAX,
    BD_OPINION_R >= OPINION_MIN,
    % AHORRO Y RESULTADO
    AHORRO is (PRESUPUESTO - COSTO_TOTAL),
    mostrar_resultados('RESULTADO. CRITERIO PRESUPUESTO ECONOMICO', BD_NOMBRE_H, BD_NOMBRE_D, BD_DIREC_H, COSTO_TRANSPORTE, COSTO_HABITACION, COSTO_COMIDA, COSTO_TOTAL, AHORRO)
).

inf_medio_presupuesto_medio(PRESUPUESTO, CLIMA, DISTANCIA_MAX, ESTRELLAS, CANTIDAD_PERSONAS, TIPO_HABITACION, CANTIDAD_DIAS, TIEMPOS_COMIDA, VEHICULO) :- (
    % BUSQUEDA DE HECHOS
    hotel(_, BD_NOMBRE_H, BD_DIREC_H, BD_ESTRELLAS_H, BD_HAB_SIMPLE_H, BD_HAB_DOBLE_H, BD_PRECIO_COMIDA_H, ID_DEPARTAMENTO, BD_DIS_H),
    departamento(ID_DEPARTAMENTO, BD_NOMBRE_D, _, _, BD_CLIMA, BD_PASAJE_D),
    % CALCULOS
    (
        VEHICULO == 's' -> COSTO_TRANSPORTE is (BD_DIS_H * 12.50 * 2);
        VEHICULO == 'n' -> COSTO_TRANSPORTE is (BD_PASAJE_D * 2 * CANTIDAD_PERSONAS)
    ...
    COSTO_TOTAL =< PRESUPUESTO,
    BD_CLIMA == CLIMA,
    BD_DIS_H =< DISTANCIA_MAX,
    BD_ESTRELLAS_H >= ESTRELLAS,
    % AHORRO Y RESULTADO
    AHORRO is (PRESUPUESTO - COSTO_TOTAL),
    mostrar_resultados('RESULTADO. CRITERIO PRESUPUESTO MEDIO', BD_NOMBRE_H, BD_NOMBRE_D, BD_DIREC_H, COSTO_TRANSPORTE, COSTO_HABITACION, COSTO_COMIDA, COSTO_TOTAL, AHORRO)
).

inf_presupuesto_alto(PRESUPUESTO, CLIMA, CLIENTES_EXTRANJEROS, CHEF, CANTIDAD_PERSONAS, TIPO_HABITACION, CANTIDAD_DIAS, TIEMPOS_COMIDA, VEHICULO) :- (
    % BUSQUEDA DE HECHOS
    hotel(ID_HOTEL, BD_NOMBRE_H, BD_DIREC_H, _, BD_HAB_SIMPLE_H, BD_HAB_DOBLE_H, BD_PRECIO_COMIDA_H, ID_DEPARTAMENTO, BD_DIS_H),
    departamento(ID_DEPARTAMENTO, BD_NOMBRE_D, _, _, BD_CLIMA, BD_PASAJE_D),
    registro(_, ID_CLIENTE, ID_HOTEL, _, _, _),
    cliente(ID_CLIENTE, _, _, CLIENTES_EXTRANJEROS, _, _, _),
    trabajador(_, _, CHEF, ID_HOTEL),
    % CALCULOS
    (
        VEHICULO == 's' -> COSTO_TRANSPORTE is (BD_DIS_H * 12.50 * 2);
        VEHICULO == 'n' -> COSTO_TRANSPORTE is (BD_PASAJE_D * 2 * CANTIDAD_PERSONAS)
    ...
    COSTO_TOTAL =< PRESUPUESTO,
    BD_CLIMA == CLIMA,
    % BD_PAIS_C == CLIENTES_EXTRANJEROS,
    % BD_CARGO_T == CHEF,
    % AHORRO Y RESULTADO
    AHORRO is (PRESUPUESTO - COSTO_TOTAL),
    mostrar_resultados('RESULTADO. CRITERIO PRESUPUESTO ALTO', BD_NOMBRE_H, BD_NOMBRE_D, BD_DIREC_H, COSTO_TRANSPORTE, COSTO_HABITACION, COSTO_COMIDA, COSTO_TOTAL, AHORRO)
).

inf_idioma(IDIOMA, DISTANCIA_MAX, TIEMPO_MAX, CLIMA, CANTIDAD_PERSONAS, TIPO_HABITACION, CANTIDAD_DIAS, TIEMPOS_COMIDA, VEHICULO) :- (
    % BUSQUEDA DE HECHOS
    hotel(_, BD_NOMBRE_H, BD_DIREC_H, _, BD_HAB_SIMPLE_H, BD_HAB_DOBLE_H, BD_PRECIO_COMIDA_H, ID_DEPARTAMENTO, BD_DIS_H),
    departamento(ID_DEPARTAMENTO, BD_NOMBRE_D, BD_TIEMPO_D, BD_IDIOMA_D, BD_CLIMA, BD_PASAJE_D),
    % CALCULOS
    (
        VEHICULO == 's' -> COSTO_TRANSPORTE is (BD_DIS_H * 12.50 * 2);
        VEHICULO == 'n' -> COSTO_TRANSPORTE is (BD_PASAJE_D * 2 * CANTIDAD_PERSONAS)
    ...
    BD_IDIOMA_D == IDIOMA,
    BD_DIS_H =< DISTANCIA_MAX,
    BD_TIEMPO_D =< TIEMPO_MAX,
    BD_CLIMA == CLIMA,
    mostrar_resultados('RESULTADO. CRITERIO IDIOMA', BD_NOMBRE_H, BD_NOMBRE_D, BD_DIREC_H, COSTO_TRANSPORTE, COSTO_HABITACION, COSTO_COMIDA, COSTO_TOTAL, 'N/A')
).
...
inf_pocas_estrellas(ESTRELLAS, COMIDA_MAX, DISTANCIA_MAX, HABITACION_MAX, CANTIDAD_PERSONAS, TIPO_HABITACION, CANTIDAD_DIAS, TIEMPOS_COMIDA, VEHICULO) :- (
    ...
    BD_DIS_H =< DISTANCIA_MAX,
    COSTO_HABITACION =< HABITACION_MAX,
    mostrar_resultados('RESULTADO. CRITERIO POCAS ESTRELLAS', BD_NOMBRE_H, BD_NOMBRE_D, BD_DIREC_H, COSTO_TRANSPORTE, COSTO_HABITACION, COSTO_COMIDA, COSTO_TOTAL, 'N/A')
).

inf_muchas_estrellas(ESTRELLAS, CHEF, CLIMA, IDIOMA, CANTIDAD_PERSONAS, TIPO_HABITACION, CANTIDAD_DIAS, TIEMPOS_COMIDA, VEHICULO) :- (
    % BUSQUEDA DE HECHOS
    hotel(ID_HOTEL, BD_NOMBRE_H, BD_DIREC_H, BD_ESTRELLAS_H, BD_HAB_SIMPLE_H, BD_HAB_DOBLE_H, BD_PRECIO_COMIDA_H, ID_DEPARTAMENTO, BD_DIS_H),
    departamento(ID_DEPARTAMENTO, BD_NOMBRE_D, _, BD_IDIOMA_D, BD_CLIMA, BD_PASAJE_D),
    trabajador(_, _, CHEF, ID_HOTEL),
    % CALCULOS
    (
        VEHICULO == 's' -> COSTO_TRANSPORTE is (BD_DIS_H * 12.50 * 2);
    ...
    BD_CLIMA == CLIMA,
    BD_IDIOMA_D == IDIOMA,
    mostrar_resultados('RESULTADO. CRITERIO MUCHAS ESTRELLAS', BD_NOMBRE_H, BD_NOMBRE_D, BD_DIREC_H, COSTO_TRANSPORTE, COSTO_HABITACION, COSTO_COMIDA, COSTO_TOTAL, 'N/A')
).

inf_clima(CLIMA, OPINION_MIN, TIEMPO_MAX, CANTIDAD_PERSONAS, TIPO_HABITACION, CANTIDAD_DIAS, TIEMPOS_COMIDA, VEHICULO) :- (
    % BUSQUEDA DE HECHOS
    hotel(ID_HOTEL, BD_NOMBRE_H, BD_DIREC_H, _, BD_HAB_SIMPLE_H, BD_HAB_DOBLE_H, BD_PRECIO_COMIDA_H, ID_DEPARTAMENTO, BD_DIS_H),
    departamento(ID_DEPARTAMENTO, BD_NOMBRE_D, BD_TIEMPO_D, _, BD_CLIMA, BD_PASAJE_D),
    registro(_, _, ID_HOTEL, _, _, BD_OPINION_R),
    % CALCULOS
    (
        VEHICULO == 's' -> COSTO_TRANSPORTE is (BD_DIS_H * 12.50 * 2);
    ...
    BD_OPINION_R >= OPINION_MIN,
    BD_TIEMPO_D =< TIEMPO_MAX,
    mostrar_resultados('RESULTADO. CRITERIO CLIMA', BD_NOMBRE_H, BD_NOMBRE_D, BD_DIREC_H, COSTO_TRANSPORTE, COSTO_HABITACION, COSTO_COMIDA, COSTO_TOTAL, 'N/A')
).

mostrar_resultados(TITULO, BD_NOMBRE_H, BD_NOMBRE_D, BD_DIREC_H, COSTO_TRANSPORTE, COSTO_HABITACION, COSTO_COMIDA, COSTO_TOTAL, AHORRO) :- nl,
    format('******************************************************************
~a
    ...
    - Ahorro:           ~a
******************************************************************
', [TITULO, BD_NOMBRE_H, BD_NOMBRE_D, BD_DIREC_H, COSTO_TRANSPORTE, COSTO_HABITACION, COSTO_COMIDA, COSTO_TOTAL, AHORRO]), nl.

% -----------------------------------------
% ---------- SECCION DE REPORTES ----------
% -----------------------------------------
menu_reportes :- nl,
    format('\t[1] Nombre y nacionalidad de clientes nacidos en Europa con opiniones mayores de 5
    \t[2] Nombre y estado civil de clientes con reservaciones en hoteles con mas de 4 estrellas
    \t[3] Nombre de Administradores en hoteles con valoraciones mayor o igual que 5
    \t[4] Departamentos y hotel con mas reservaciones, en clima calor
    \t[5] Nombre de clientes extranjeros hospedados en departamentos de habla espanol
    \t[6] Nombre de Hotel, departamento, idioma y Nombre de clientes con opiniones mayores o igual que 7 y estadias mayores o igual a 3 dias
    \t[7] Nombre Pais y Nombre de Hotel de clientes extranjeros hospedados en departamentos de habla ingles, con menos de 2 dias de hospedaje
    \t[8] Pais que reservo en Peten
    \t[9] Nombre de Hotel y direccion de hoteles que recibieron a clientes casados, que tengan opiniones mayores de 6 por personas con minimo de 3 dias de estadia
    -> '),
    read(OP_REPORTE), seccion_reportes(OP_REPORTE).

seccion_reportes(OP_REPORTE) :- (
...
reporte_1 :- write('***********************  REPORTE 1  ***********************'), nl,
    write('Nombre y nacionalidad de clientes nacidos en Europa con opiniones mayores de 5'), nl, nl,
    write('\tNombre   |   Apellido    |   Nacionalidad    |   Opinion'), nl,
    write('\t--------------------------------------------------------'), nl,
    cliente(ID_CLIENTE, BD_NOMBRE_C, BD_APELLIDO_C, BD_PAIS_C, _, _, _),
    registro(_, ID_CLIENTE, _, _, _, BD_OPINION_R),
    BD_OPINION_R > 5,
    not(BD_PAIS_C == 'Guatemala'; BD_PAIS_C == 'El Salvador'; BD_PAIS_C == 'Honduras'; BD_PAIS_C == 'Venezuela'; BD_PAIS_C == 'Mexico'; BD_PAIS_C == 'Costa Rica'),
    format('\t~a  |   ~a  |   ~a  |   ~a\n', [BD_NOMBRE_C, BD_APELLIDO_C, BD_PAIS_C, BD_OPINION_R]),
    fail.

reporte_2 :- write('***********************  REPORTE 2  ***********************'), nl,
    write('Nombre y estado civil de clientes con reservaciones en hoteles con mas de 4 estrellas'), nl, nl,
    write('\tNombre  |   Apellido    |   Estado Civil'), nl,
    write('\t----------------------------------------'), nl,
    cliente(ID_CLIENTE, BD_NOMBRE_C, BD_APELLIDO_C, _, _, BD_ESTADO_C, _),
    hotel(ID_HOTEL, _, _, BD_ESTRELLAS_H, _, _, _, _, _),
    registro(_, ID_CLIENTE, ID_HOTEL, _, _, _),
    BD_ESTRELLAS_H > 4,
    format('\t~a  |   ~a  |   ~a\n', [BD_NOMBRE_C, BD_APELLIDO_C, BD_ESTADO_C]),
    fail.

reporte_3 :- write('***********************  REPORTE 3  ***********************'), nl,
    write('Nombre de Administradores en hoteles con valoraciones mayor o igual que 5'), nl, nl,
    write('\tAdministrador   |   Hotel    |   Cargo    |   Valoracion'), nl,
    write('\t--------------------------------------------------------'), nl,
    trabajador(_, BD_NOMBRE_T, BD_CARGO_T, ID_HOTEL),
    hotel(ID_HOTEL, BD_NOMBRE_H, _, _, _, _, _, _, _),
    registro(_, _, ID_HOTEL, _, _, BD_OPINION_R),
    BD_CARGO_T == 'Administrador',
    BD_OPINION_R >= 5,
    format('\t~a  |   ~a  |   ~a   |   ~a\n', [BD_NOMBRE_T, BD_NOMBRE_H, BD_CARGO_T, BD_OPINION_R]),
    fail.

reporte_4 :- write('***********************  REPORTE 4  ***********************'), nl,
    write('Departamentos y hotel con mas reservaciones, en clima calor'), nl, nl,
    write('\tDepartamento   |   Hotel    |   Clima'), nl,
    write('\t-------------------------------------'), nl,
    departamento(ID_DEPARTAMENTO, BD_NOMBRE_D, _, _, BD_CLIMA, _),
    registro(_, _, ID_HOTEL, _, _, _),
    hotel(ID_HOTEL, BD_NOMBRE_H, _, _, _, _, _, ID_DEPARTAMENTO, _),
    BD_CLIMA == 'calor',
    format('\t~a  |   ~a  |   ~a\n', [BD_NOMBRE_D, BD_NOMBRE_H, BD_CLIMA]),
    fail.

reporte_5 :- write('***********************  REPORTE 5  ***********************'), nl,
    write('Nombre de clientes extranjeros hospedados en departamentos de habla espanol'), nl, nl,
    write('\tNombre   |   Apellido    |    Nacionalidad    |    Departamento    |   Idioma'), nl,
    write('\t-----------------------------------------------------------------------------'), nl,
    cliente(ID_CLIENTE, BD_NOMBRE_C, BD_APELLIDO_C, BD_PAIS_C, _, _, _),
    hotel(ID_HOTEL, _, _, _, _, _, _, ID_DEPARTAMENTO, _),
    registro(_, ID_CLIENTE, ID_HOTEL, _, _, _),
    departamento(ID_DEPARTAMENTO, BD_NOMBRE_D, _, BD_IDIOMA_D, _, _),
    BD_IDIOMA_D == 'espanol',
    not(BD_PAIS_C == 'Guatemala'),
    format('\t~a  |   ~a  |   ~a  |   ~a   |   ~a\n', [BD_NOMBRE_C, BD_APELLIDO_C, BD_PAIS_C, BD_NOMBRE_D, BD_IDIOMA_D]),
    fail.

reporte_6 :- write('***********************  REPORTE 6  ***********************'), nl,
    write('Nombre de Hotel, departamento, idioma y Nombre de clientes con opiniones mayores o igual que 7 y estadias mayores o igual a 3 dias'), nl, nl,
    write('\tHotel   |   Departamento    |   Idioma    |   Nombre   |   Apellido   |    Estadia    |    Opinion'), nl,
    write('\t--------------------------------------------------------------------------------------------------'), nl,
    cliente(ID_CLIENTE, BD_NOMBRE_C, BD_APELLIDO_C, _, _, _, _),
    hotel(ID_HOTEL, BD_NOMBRE_H, _, _, _, _, _, ID_DEPARTAMENTO, _),
    registro(_, ID_CLIENTE, ID_HOTEL, _, BD_ESTADIA_R, BD_OPINION_R),
    departamento(ID_DEPARTAMENTO, BD_NOMBRE_D, _, BD_IDIOMA_D, _, _),
    BD_OPINION_R >= 7,
    BD_ESTADIA_R >= 3,
    format('\t~a  |   ~a  |   ~a   |   ~a  |   ~a  \t|   ~a   \t|   ~a\n', [BD_NOMBRE_H, BD_NOMBRE_D, BD_IDIOMA_D, BD_NOMBRE_C, BD_APELLIDO_C, BD_ESTADIA_R, BD_OPINION_R]),
    fail.

reporte_7 :- write('***********************  REPORTE 7  ***********************'), nl,
    write('Nombre Pais y Nombre de Hotel de clientes extranjeros hospedados en departamentos de habla ingles, con menos de 2 dias de hospedaje'), nl, nl,
    write('\tPais   |   Hotel    |   Nombre    |   Apellido    |    Departamento    |   Idioma   |   Estadia'), nl,
    write('\t-----------------------------------------------------------------------------------------------'), nl,
    cliente(ID_CLIENTE, BD_NOMBRE_C, BD_APELLIDO_C, BD_PAIS_C, _, _, _),
    hotel(ID_HOTEL, BD_NOMBRE_H, _, _, _, _, _, ID_DEPARTAMENTO, _),
    registro(_, ID_CLIENTE, ID_HOTEL, _, BD_ESTADIA_R, _),
    departamento(ID_DEPARTAMENTO, BD_NOMBRE_D, _, BD_IDIOMA_D, _, _),
    BD_ESTADIA_R < 2,
    BD_IDIOMA_D == 'ingles',
    format('\t~a  |   ~a  |   ~a   |   ~a  |   ~a  \t|   ~a   \t|   ~a\n', [BD_PAIS_C, BD_NOMBRE_H, BD_NOMBRE_C, BD_APELLIDO_C, BD_NOMBRE_D, BD_IDIOMA_D, BD_ESTADIA_R]),
    fail.

reporte_8 :- write('***********************  REPORTE 8  ***********************'), nl,
    write('Pais que reservo en Peten'), nl, nl,
    write('\tPais   |   Departamento'), nl,
    write('\t-----------------------'), nl,
    cliente(ID_CLIENTE, _, _, BD_PAIS_C, _, _, _),
    hotel(ID_HOTEL, _, _, _, _, _, _, ID_DEPARTAMENTO, _),
    registro(_, ID_CLIENTE, ID_HOTEL, _, _, _),
    departamento(ID_DEPARTAMENTO, BD_NOMBRE_D, _, _, _, _),
    BD_NOMBRE_D == 'Peten',
    format('\t~a  |   ~a\n', [BD_PAIS_C, BD_NOMBRE_D]),
    fail.

reporte_9 :- write('***********************  REPORTE 9  ***********************'), nl,
    write('Nombre de Hotel y direccion de hoteles que recibieron a clientes casados, que tengan opiniones mayores de 6 por personas con minimo de 3 dias de estadia'), nl, nl,
    write('\tHotel   |   Direccion   |   Nombre    |   Apellido     |    Estado Civil    |    Opinion    |    Estadia'), nl,
    write('\t---------------------------------------------------------------------------------------------------------------'), nl,
    cliente(ID_CLIENTE, BD_NOMBRE_C, BD_APELLIDO_C, _, _, BD_ESTADO_C, _),
    hotel(ID_HOTEL, BD_NOMBRE_H, BD_DIREC_H, _, _, _, _, _, _),
    registro(_, ID_CLIENTE, ID_HOTEL, _, BD_ESTADIA_R, BD_OPINION_R),
    BD_ESTADO_C == 'Casado',
    BD_OPINION_R > 6,
    BD_ESTADIA_R >= 3,
    format('\t~a  |   ~a  \t|   ~a   |   ~a  \t|   ~a  \t\t|   ~a   \t|   ~a\n', [BD_NOMBRE_H, BD_DIREC_H, BD_NOMBRE_C, BD_APELLIDO_C, BD_ESTADO_C, BD_OPINION_R, BD_ESTADIA_R]),
    fail.
